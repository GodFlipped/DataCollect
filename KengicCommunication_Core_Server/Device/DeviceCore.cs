    ...
    public class DeviceCore
    {
        ...
        private void ParseFromRequest( byte[] data, DeviceRequest request )
        {
            foreach (var regular in request.RegularNodes)
            {
                ...
                jsonLock.Enter( );
                if (regular.RegularCode != RegularNodeTypeItem.FStringAscii().Code &&
                    regular.RegularCode != RegularNodeTypeItem.FStringUnicode().Code &&
                    regular.RegularCode != RegularNodeTypeItem.FStringUtf8().Code &&
