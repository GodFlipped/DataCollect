...
{
    public class MQTTnet48Power : BackgroundService
    {
        ...
        public MQTTnet48Power(ILogger<MQTTnet48Power> logger, MQTTnetClient mQTTnetClient)
        {
            this._logger = logger;
        ...
        public void OnTimedEvent(object source, ElapsedEventArgs e)
        {
            try
            {
                ...
                {
                    var propertiesHeader = new MqttReport48VProperties1D
                    {
                        ...
                        properties = new MachineBasicInformation48V1D
                        {
                            powerDeviceBaseInfo = new List<PowerDeviceBaseInfo>(),
                    ...
                    foreach (var item in ListKye)
                    {
                        ...
                        //设备总数上传
                        if (variable.DeviceType == "SDProperty" && variable.OpcValue == "ST-SD-设备总数")
                        {
                            propertiesHeader.properties.powerDeviceAmount = Convert.ToInt16(variable.ComponentProperty);
                        }
                        //设备基础信息上传
                        if (variable.DeviceType == "SDProperty" && variable.ComponentPropertyType == "设备基础信息")
                        {
                            var ComponentPropertys = variable.ComponentProperty.Split(';');
                            propertiesHeader.properties.powerDeviceBaseInfo.Add(new PowerDeviceBaseInfo
                            {
                ...
                {
                    var propertiesHeader = new MqttReport48VProperties4S
                    {
                        ...
                        deviceId = _deviceId,
                        timestamp = timeToLong10,
                        properties = new MachineBasicInformation48V4S
                        {
                            powerFaultStatus = new List<PowerFaultStatus>()
                        }
                    };
                    foreach (var item in ListKye)
                    {
                        var variable = RedisConn.Instance.rds.Get<Variable>(item.OpcValue);
                        //设备设备故障状态上传
                        if (variable.DeviceType == "SDError")
                        {
                            var haveError = "0";
                            ...
                            propertiesHeader.properties.powerFaultStatus.Add(new PowerFaultStatus
                            {
                                componentNo = variable.DeviceNumber,
