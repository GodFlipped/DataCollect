...
using KengicCommunication_Core.ModBus;
...
namespace SharpNodeSettings.Core
{
    ...
    public class SharpNodeServer
    {
        ...
        public SharpNodeServer()
        {
            ...
            settingsLock = new KengicCommunication_Core.Core.SimpleHybirdLock();
        }
        ...
        private void SimplifyServer_ReceiveStringEvent(AppSession session, KengicCommunication_Core.NetHandle handle, string data)
        {
            if (handle == 0)
            {
                // 请求配置文件
            ...
            else if (handle == 1)
            {
                ...
                //logNet?.WriteDebug( "请求设备信息：" + data+";结果="+ response);
                simplifyServer.SendMessage(session, handle, response);
            }
            else if (handle == 2)
            {
                string[] nodeData = data.Split(new char[] { '&' });
                // 请求设备的所有数据
                string[] nodePath = nodeData[0].Split(new char[] { '\\', ':', '-', '.', '_', '/' });
                string response = string.Empty;
                ...
                for (int i = 0; i < deviceCores.Count; i++)
                {
                    if (deviceCores[i].IsCurrentDevice(nodePath))
                    {
                        ...
                        foreach (var Request in deviceCores[i].Requests)
                        {
                            foreach (var regular in Request.RegularNodes)
                            {
                                if (regular.Name == nodePath[nodePath.Length - 1])
                                {
                                    string[] Address = Request.Address.Split('.');
                                    string AddressData = Address[0].ToString() + "." + (Convert.ToInt64(Address[1]) + regular.Index).ToString();
                                    if (regular.RegularCode == RegularNodeTypeItem.FBool().Code)
                                    {
                                        string val = nodeData[1];
                                        if (nodeData[1] == "1")
                                        {
                                            val = "True";
                                        }
                                        else if (nodeData[1] == "0")
                                        {
                                            val = "False";
                                        }

                                        deviceCores[i].ReadWriteDevice.Write(AddressData, bool.Parse(val));
                                    }
                                    else if (regular.RegularCode == RegularNodeTypeItem.FInt16().Code)
                                    {
                                      
                                        deviceCores[i].ReadWriteDevice.Write(AddressData, short.Parse(nodeData[1]));

                                    }
                                    else if (regular.RegularCode == RegularNodeTypeItem.FInt32().Code)
                                    {
                                        
                                        deviceCores[i].ReadWriteDevice.Write(AddressData, int.Parse(nodeData[1]));
                                    }
                                }
                ...
                //logNet?.WriteDebug("写入设备信息：" + data + ";结果=" + nodeData[1]);
                simplifyServer.SendMessage(session, handle, response);
            }
            else
            {
        ...
        private KengicCommunication_Core.Core.SimpleHybirdLock settingsLock;                       // 配置文件加载的锁
