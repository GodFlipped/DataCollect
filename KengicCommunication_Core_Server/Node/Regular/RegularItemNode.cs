...
namespace SharpNodeSettings.Node.Regular
{
    ...
    public class RegularItemNode : NodeClass, IComparable<RegularItemNode>
    {
        ...
        public RegularItemNode( )
        {
            ...
            //logNet.WriteAnyString("&&&&&&&" + RegularNodeTypeItem.FInt16().Code);
        ...
        /// 类型的代号，详细参见const数据
        /// </summary>
        public int RegularCode { get; set; } = RegularNodeTypeItem.FInt16().Code;
        ...
        {
            if (RegularCode == RegularNodeTypeItem.FBool().Code)
            {
                return Index / 8;
            }
            else
        ...
        public dynamic GetValue( byte[] data, IByteTransform byteTransform )
        {
            ...
            if (RegularCode == RegularNodeTypeItem.FBool().Code)
            {
                if (TypeLength == 1)
                {
                    bool[] tmp = KengicCommunication_Core.BasicFramework.SoftBasic.ByteToBoolArray( data, data.Length * 8 );
                    return tmp[Index];
                }
                else
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FInt16().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FUInt16().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FInt32().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FUInt32().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FInt64().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FUInt64().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FFloat().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FDouble().Code)
            {
                if (TypeLength == 1)
                {
            ...
            else if (RegularCode == RegularNodeTypeItem.FStringAscii().Code)
            {
                return Encoding.ASCII.GetString( data, Index, TypeLength );
            }
            else if (RegularCode == RegularNodeTypeItem.FStringUtf8().Code)
            {
                return Encoding.UTF8.GetString(data, Index, TypeLength);
            }
            else
            {
        ...
        public int GetLengthByte( )
        {
            if (RegularCode == RegularNodeTypeItem.FBool().Code)
            {
                ...
                RegularCode == RegularNodeTypeItem.FStringUnicode().Code ||
                RegularCode == RegularNodeTypeItem.FStringUtf8().Code)
            {
                return TypeLength + Index;
            }
            else if (RegularCode == RegularNodeTypeItem.FUInt16().Code ||
                RegularCode == RegularNodeTypeItem.FInt16().Code)
            {
                return TypeLength * 2 + Index;
            }
            else if (RegularCode == RegularNodeTypeItem.FInt32().Code ||
                RegularCode == RegularNodeTypeItem.FUInt32().Code ||
                RegularCode == RegularNodeTypeItem.FFloat().Code)
            {
                return TypeLength * 4 + Index;
            }
            else if (RegularCode == RegularNodeTypeItem.FInt64().Code ||
                RegularCode == RegularNodeTypeItem.FUInt64().Code ||
                RegularCode == RegularNodeTypeItem.FDouble().Code)
            {
