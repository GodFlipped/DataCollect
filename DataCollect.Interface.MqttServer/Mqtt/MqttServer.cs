using DataCollect.Application.Service.Mqtt.Enums;
...
namespace DataCollect.Interface.MqttServer.Mqtt.Dtos
{
    /// <summary>
    /// Mqtt服务端对象
    /// </summary>
    public class MqttServerConnect : ISingleton
	{
		...
		private void MqttServer_OnClientApplicationMessageReceive(MqttSession session, MqttClientApplicationMessage message)
		{
			...
			{
				_log.LogInformation($"Cliend Id[{message.ClientId}] Topic:[{message.Topic}] Payload:[{Encoding.UTF8.GetString(message.Payload.SelectBegin(100))}...]" + Environment.NewLine);
			}
			else
			{
				_log.LogInformation($"Cliend Id[{message.ClientId}] Topic:[{message.Topic}] Payload:[{Encoding.UTF8.GetString(message.Payload)}]" + Environment.NewLine);
			}
